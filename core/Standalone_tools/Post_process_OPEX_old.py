...
# Ensure results folder exists (create if missing)
...
for macro_name, data in macro_data.items():
    ...
    if total_opex_arr.size > 0:
        ...
        bins = min(N_BINS, max(1, vals.size))
        ...
        ax.legend()
        out_path = RESULTS_FOLDER_fig / f"histogram_total_opex_{safe_name}.png"
        plt.tight_layout()
    ...
    if annual_opex_arr.size > 0:
        ...
        ax.grid(True, linestyle="--", alpha=0.3)
        ax.legend()
        out_path = RESULTS_FOLDER_fig / f"histogram_annual_opex_{safe_name}.png"
        plt.tight_layout()
        plt.savefig(out_path, dpi=150)
        plt.close()
...
if nonempty_total:
    ...
    bins = min(N_BINS, max(5, len(all_vals)))
    ...
    ax.grid(True, linestyle="--", alpha=0.3)
    ax.legend()
    out_path = RESULTS_FOLDER_fig / "histogram_total_opex_all_scenarios.png"
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
...
if nonempty_ann:
    ...
    bins_ann = min(N_BINS, max(5, len(all_vals_ann)))
    ...
    ax.grid(True, linestyle="--", alpha=0.3)
    ax.legend()
    out_path = RESULTS_FOLDER_fig / "histogram_annual_opex_all_scenarios.png"
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
...
N_BINS = 60        # increase for finer bins (e.g. 50–100)
MIN_BINS = 10
...
for macro_name, data in macro_data.items():
    ...
    if npv_equity_arr.size > 0:
        ...
        out_path = RESULTS_FOLDER_fig / f"histogram_npv_equity_{safe_name}.png"
        plt.tight_layout()
        plt.savefig(out_path, dpi=150)
        plt.close()
    ...
    if npv_firm_arr.size > 0:
        ...
        out_path = RESULTS_FOLDER_fig / f"histogram_npv_firm_{safe_name}.png"
        plt.tight_layout()
        plt.savefig(out_path, dpi=150)
        plt.close()
...
if nondet_eq or det_eq:
    ...
    if nondet_eq:
        ...
        bins_eq = min(N_BINS, max(5, len(all_vals)))
    ...
    ax.legend()
    out_path = RESULTS_FOLDER_fig / "histogram_npv_equity_all_scenarios.png"
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
...
if nondet_f or det_f:
    ...
    if nondet_f:
        ...
        bins_f = min(N_BINS, max(5, len(all_vals)))
    ...
    ax.legend()
    out_path = RESULTS_FOLDER_fig / "histogram_npv_firm_all_scenarios.png"
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
